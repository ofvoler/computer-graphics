...
class GLUtils {
    ...
    static createShaderProgram(gl, vertexShaderScr, fragmentShaderSrc) {
        ...
        gl.bindAttribLocation(shaderProgram, A_TEXCOORDS_LOC, A_TEXCOORDS_NAME);
    ...
    static createMeshVAO(gl, name, drawMode, vertexArray, normalArray, texcoordsArray, indexArray) {
        ...
        if (normalArray !== undefined && normalArray != null) {
            ...
            mesh.normalComponentLen = 3; //How many floats make up a vertex
        ...
        if (texcoordsArray !== undefined && texcoordsArray != null) {
            ...
            gl.enableVertexAttribArray(A_TEXCOORDS_LOC); //Enable Attribute location
            gl.vertexAttribPointer(A_TEXCOORDS_LOC, 2, gl.FLOAT, false, 0, 0); //Put buffer at location of the vao
        }
    ...
    static loadTexture(gl, name, textureSource) {
        let textureHandler = gl.createTexture();
        gl.bindTexture(gl.TEXTURE_2D, textureHandler); // currently active texture2D

        // fill texture with data
        gl.texImage2D(
            gl.TEXTURE_2D, // texture binding point
            0, // level of detail
            gl.RGBA, // internal format
            1, 1, // width and height of texture
            0, // border
        ...
        let image = new Image();
        image.onload = function() { // function executes when source is being loaded
            gl.bindTexture(gl.TEXTURE_2D, textureHandler); // bind again so this texture is currently active
        ...
        image.onerror = function() {
            console.error("The image couldn't be loaded!!! Image: " + textureSource);
        }

        image.src = textureSource;
        TextureCache[name] = textureHandler; // save texture in cache for later use
    }
